...
if (file.exists("counts_ccRCC_wang_2020.csv")){cts <- read.csv(file = "Table counts.csv", row.names = 1)
...
if (identical(meta[order(meta[,"Run"]), "Run"], colnames(cts)))
{colnames(cts) <- meta[order(meta[, "Run"]), "Sample.Name"]}
...
if(!exists("wrs_res"))
{ wrs_res <- wrs_res_gen(cts, meta)} #Wilcoxon rank sum test for differential expressed genes

annotation_color_list <- ann_colors() #for future heatmaps

gene_lists <- gene_lists_gen(cts, wrs_res) #for pca and more
...
if (!exists("dsq_normal_cancer"))
{
  ...
  dsq_nc_t <- DESeqDataSetFromMatrix(countData = cts,
                                              colData = meta,
                                              design= ~ Thrombosis)
